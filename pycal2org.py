...
def format_dateinterval(dtstart, dtend=None):
...
def generate_dates(event):
    ...
    if not 'rrule' in event:
        dates.append(format_dateinterval(dtstart, dtend))
    # Handle recurrent events
        ...
        for prop in ['rdate', 'exdate']:
            if not prop in event:
                continue
            # This can return either a single value or a list, so it's a
            # mess...
        ...
        for dtstart_repeat in ruleset:
            ...
            dates.append(format_dateinterval(dtstart_repeat, dtend_repeat))
    return dates

def generate_org_fragment(event):
    s = ''
    s += '* {}\n'.format(event['summary'])
    for date in generate_dates(event):
        s += '  {}\n'.format(date)
    s += '\n'
